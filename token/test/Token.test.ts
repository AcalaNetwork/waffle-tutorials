    ...
    describe('Transfer', () => {
      describe('transfer()', () => {
        ...
        it('should emit Transfer event', async () => {
          await expect(instance.connect(deployer).transfer(userAddress, 500))
            .to.emit(instance, 'Transfer')
            .withArgs(deployerAddress, userAddress, 500);
        });

        it('should revert when trying to transfer to 0x0 address', async () => {
          await expect(instance.connect(deployer).transfer(NULL_ADDRESS, 500)).to.be.revertedWith(
            'ERC20: transfer to the zero address'
          );
        });

        it('should revert when trying to transfer more than own balance', async () => {
          await expect(instance.connect(user).transfer(deployerAddress, 1234567890)).to.be.revertedWith(
    ...
    describe('Allowances', () => {
      describe('approve()', () => {
        ...
        it('should emit Approval', async () => {
          await expect(instance.connect(deployer).approve(userAddress, 1500))
        ...
        it('should revert when trying to grant allowance to 0x0 address', async () => {
          await expect(instance.connect(deployer).approve(NULL_ADDRESS, 1500)).to.be.revertedWith(
      ...
      describe('increaseAllowance()', () => {
        ...
        it('should emit Approval event', async () => {
          ...
          await expect(instance.connect(deployer).increaseAllowance(userAddress, 500))
            .to.emit(instance, 'Approval')
            .withArgs(deployerAddress, userAddress, 1500);
        });
      ...
      describe('decreaseAllowance()', () => {
        ...
        it('should emit Approval event', async () => {
          ...
          await expect(instance.connect(deployer).decreaseAllowance(userAddress, 500))
            .to.emit(instance, 'Approval')
            .withArgs(deployerAddress, userAddress, 1000);
        });

        it('should revert when trying to reduce the allowance below 0', async () => {
          await expect(instance.connect(deployer).decreaseAllowance(userAddress, 10000)).to.be.revertedWith(
      ...
      describe('transferFrom()', () => {
        ...
        it('should emit Transfer', async () => {
          ...
          await expect(instance.connect(user).transferFrom(deployerAddress, userAddress, 500))
            .to.emit(instance, 'Transfer')
            .withArgs(deployerAddress, userAddress, 500);
        });
        ...
        it('should revert when trying to transfer amount higher than allowance', async () => {
          ...
          await expect(instance.connect(user).transferFrom(deployerAddress, userAddress, 100000)).to.be.revertedWith(
        ...
        it('should rever when trying to transfer to 0x0 address', async () => {
          ...
          await expect(instance.connect(user).transferFrom(deployerAddress, NULL_ADDRESS, 500)).to.be.revertedWith(
            'ERC20: transfer to the zero address'
          );
        });
        ...
        it('should revert when allowance was not given', async () => {
          await expect(instance.connect(deployer).transferFrom(userAddress, deployerAddress, 1500)).to.be.revertedWith(
            'ERC20: transfer amount exceeds allowance'
          );
        });
      });
